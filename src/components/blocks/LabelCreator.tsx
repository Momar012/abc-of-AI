...
export default function LabelCreator({ blockId, labels }: LabelCreatorProps) {
  ...
  const handleAdd = () => {
    ...
    const isDuplicate = labels.some((l) => l.name.toLowerCase() === trimmed.toLowerCase())
    if (isDuplicate) { setError(`"${trimmed}" already exists`); return }
    setError('')
    addLabel(blockId, trimmed || undefined)
    setName('')
  }
    ...
    <div className="flex flex-col gap-1">
      <div className="flex gap-1.5 mt-2">
        <input
          ...
          className={`flex-1 px-2 py-1 rounded-lg bg-white/10 border text-white text-xs placeholder-white/40 outline-none transition-colors ${error ? 'border-red-400/70 focus:border-red-400' : 'border-white/15 focus:border-violet-400'}`}
          maxLength={30}
        />
        <button
          onClick={handleAdd}
